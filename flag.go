...
type ParseErrorsWhitelist struct {
	...
	UnknownFlags bool
	// PassUnknownFlagsToArgs will treat unknown flags as non-flag arguments.
...
func (f *FlagSet) parseLongArg(s string, args []string, fn parseFunc) (a []string, err error) {
	...
	if !exists {
		...
		case f.ParseErrorsWhitelist.UnknownFlags:
			if f.ParseErrorsWhitelist.PassUnknownFlagsToArgs {
	...
	for len(shorthands) > 0 {
		...
		if err != nil {
			if errUnknownFlag, ok := err.(*unknownFlagError); ok {
				// this means f.ParseErrorsWhitelist.UnknownFlags
				// and f.ParseErrorsWhitelist.PassUnknownFlagsToArgs are set.
